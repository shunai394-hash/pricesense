    ...
    >
      ...
      <LegalSection title="第2条（サービス内容）">
        ...
        <ul>
          ...
          <li>年間機会損失の試算</li>
          <li>交渉文サンプルの表示</li>
          <li>診断結果PDFの保存（メール登録時。ご依頼時のみ送信）</li>
          <li>
            Premiumプラン（月額
          ...
          <li>
            管理者向けのAI営業支援（スコア・意図・次アクションの提案。外部への営業メール送信や契約は自動実行しません）
          </li>
          <li>営業アクションの監査ログ（実行・失敗・再実行・キャンセルの記録）</li>
        </ul>
        <p>
      ...
      <LegalSection title="第5条の2（ユーザーの責任）">
        <p>
          ユーザーは、自己の責任において当サービスを利用し、入力内容・交渉文・提案Draftの利用結果について責任を負います。
          AIの出力を検証せずに契約・価格・法務判断へ用いてはなりません。
        </p>
      </LegalSection>

      <LegalSection title="第5条の3（アカウントの停止）">
        <p>
          当社は、利用規約または利用禁止事項に違反した場合、事前通知なく利用の制限または停止を行うことがあります。
          管理者トークンの漏洩が疑われる場合も、アクセスを停止することがあります。
        </p>
      </LegalSection>

      <LegalSection title="第4条の2（解約）">
        <p>
          Premiumプランの解約は、Stripeの顧客ポータルまたは当社所定の方法により行えます。
          無料の単価診断は登録なしで利用でき、利用を止める場合はブラウザの利用を終了してください。
          保存データの削除を希望する場合は、お問い合わせください。
        </p>
      </LegalSection>
